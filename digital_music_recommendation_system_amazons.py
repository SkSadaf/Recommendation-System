...
metadata = getDF('Data/Music_recom/meta_Digital_Music.json.gz')
metadata.head()
print(metadata.head())
# display the review data as  dataframe
reviews = getDF('Data/Music_recom/reviews_Digital_Music.json.gz')
reviews.head()
print(reviews.head())
...
with open('Data/Music_recom/reviews_' + dataName + '.json') as f:
    for line in f:
...
with open('Data/Music_recom/meta_' + dataName + '.json') as f:
    for line in f:
...
ex_data = open('Data/Music_recom/reviews_' + dataName + '.txt', "w")
...
ex_data = open('Data/Music_recom/meta_' + dataName + '.txt', "w")
...
class DoctoVec:
    ...
    def train(dataName, x_train, vector_size, epoch_num,kind):
        ...
        model_dm.save('Data/Music_recom/' + dataName + '_Model_'+ kind+"_"+ str(vector_size))  # saving model after we turn it to vector so we could use it in future
        return model_dm

    # save vectors
    # saving the vector for any text we will give to our model
    def saveVector(dataName, model_dm, v_size, count,kind):
        out = open('Data/Music_recom/' + dataName + '.'+kind, "w", encoding='utf-8')
...
reviews_data = []
with open('Data/Music_recom/reviews_' + dataName + '.txt') as f:
    for line in f:
        line_dict = json.dumps(eval(line))
...
def loading_metadata():
    data = []
    dict_d = {}
    with open( 'Data/Music_recom/meta_' + dataName + '.txt') as f:
        for line in f:
            line_dict = json.dumps(eval(line))
...
train_list = []
train_data = open('Data/Music_recom/' + dataName + ".train.rating", "w")
...
test_list = []
test_data = open('Data/Music_recom/'  + dataName + ".test.rating", "w")
...
path = 'Data/Music_recom/' + dataName
...
checkpoint = ModelCheckpoint('Data/Music_recom/Music_model.h5', save_best_only=True, monitor='val_loss', mode='min')
...
# user_input_test = np.save("npy_dat/music_npy/user_input_test",user_input_test)
# user_fea_test = np.save("npy_dat/music_npy/user_fea_test", user_fea_test)
# item_input_test = np.save("npy_dat/music_npy/item_input_test", item_input_test)
# item_fea_test = np.save("npy_dat/music_npy/item_fea_test", item_fea_test)
# test_label = np.save("npy_dat/music_npy/test_label", test_label)
...
# fit our model
hist = model.fit([user_input, user_fea, item_input, item_fea], labels, batch_size=256, epochs=10,
