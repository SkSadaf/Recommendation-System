...
checkpoint = ModelCheckpoint('Data/baby_recom/model.h5', save_best_only=True, monitor='val_loss', mode='min')

# user_input = np.save("npy_dat/baby_npy/user_input",user_input)
# user_fea = np.save("npy_dat/baby_npy/user_fea", user_fea)
# item_input = np.save("npy_dat/baby_npy/item_input", item_input)
# item_fea = np.save("npy_dat/baby_npy/item_fea", item_fea)
# labels = np.save("npy_dat/baby_npy/labels", labels)
...
print("MAE of Optimization model : ", mean_absolute_error(opt_model.predict([user_input, user_fea, item_input, item_fea]), labels))

print("MSE of Optimization model : ", mean_squared_error(opt_model.predict([user_input, user_fea, item_input, item_fea]), labels))
