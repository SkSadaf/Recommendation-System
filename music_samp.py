...
def user_recom_stat(vector_size, num_users, user_input1, user_fea1, labels1, epochs):
    ...
    checkpoint = ModelCheckpoint('Data/Music_recom/Music_recom_user.h5', save_best_only=True, monitor='val_loss', mode='min')
...
def item_recom_stat(vector_size, num_items, item_input1, item_fea1, labels1, epochs):
    ...
    # using callbacks to avoid overfitting and save best model during training
    earlystopping = EarlyStopping(monitor='val_loss', patience=100)
    checkpoint = ModelCheckpoint('Data/Music_recom/Music_recom_item.h5', save_best_only=True, monitor='val_loss', mode='min')
...
def uter_model(vector_size, num_users, user_input1, user_fea1, item_input1, item_fea1, labels1, epochs, opt):
    ...
    # using callbacks to avoid overfitting and save best model during training
    earlystopping = EarlyStopping(monitor='val_loss', patience=100)
    checkpoint = ModelCheckpoint('Data/Music_recom/Music_model_uter.h5', save_best_only=True, monitor='val_loss', mode='min')

    # -------------- fit our model ---------------- #
    hist = model.fit([user_input1, user_fea1, item_input1, item_fea1], labels1, batch_size=64,
    ...
    if opt == 0:
        ...
        model = model
        model = update_weights_model_BA(model, [user_input1, user_fea1, item_input1, item_fea1], labels1, 1)
