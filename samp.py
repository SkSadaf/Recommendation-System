...
def main_weight_updation_optimization(curr_wei, model, test_data, test_lab, opt):
    ...
    if opt == 1:
        print(" Bat-Inspired Optimization")
        model = BA(problem_dict1, epoch=50, pop_size=50)
    if opt == 2:
        print("Cat Swarm Optimization")
        model = CSO(problem_dict1, epoch=50, pop_size=100)
...
def user_recom_stat(vector_size, num_users, user_input1, user_fea1, labels1,user_input_test, user_fea_test, epochs):
    ...
    # fit our model
    hist = model.fit([user_input1, user_fea1], labels1, batch_size=64,
                     epochs=epochs, validation_split=0.2, verbose=1, callbacks=[earlystopping, checkpoint])
    pred = model.predict([user_input_test, user_fea_test])

    return pred


def item_recom_stat(vector_size, num_users, item_input1, item_fea1, labels1, item_input_test, item_fea_test, epochs):
    ...
    # fit our model
    hist = model.fit([item_input1, item_fea1], labels1, batch_size=64,
                     epochs=epochs, validation_split=0.2, verbose=1, callbacks=[earlystopping, checkpoint])
...
def hybrid_uter_model(vector_size, num_users, user_input1, user_fea1, item_input1, item_fea1, labels1,user_input_test, user_fea_test, item_input_test, item_fea_test, epochs, opt):
...
#
# model = user_recom_stat(vector_size, num_users, user_input, user_fea, labels, epochs)
# opt_model = item_recom_stat(vector_size, num_users, item_input, item_fea, labels, epochs)
# opt_model2 = hybrid_uter_model(vector_size, num_users, user_input, user_fea, item_input, item_fea, labels, epochs, opt=2)
#
...
#
# # --------- evaluate the test dataset ------------ #
# print("MAE of Bat Optimization model : ", mean_absolute_error(opt_model.predict([user_input_test, user_fea_test, item_input_test, item_fea_test]), test_label))
#
# print("MSE of Bat Optimization model : ", mean_squared_error(opt_model.predict([user_input_test, user_fea_test, item_input_test, item_fea_test]), test_label))
#
...
# # --------- evaluate the test dataset ------------ #
# print("MAE of Cat Optimization model : ", mean_absolute_error(opt_model2.predict([user_input_test, user_fea_test, item_input_test, item_fea_test]), test_label))
#
# print("MSE of Cat Optimization model : ", mean_squared_error(opt_model2.predict([user_input_test, user_fea_test, item_input_test, item_fea_test]), test_label))
