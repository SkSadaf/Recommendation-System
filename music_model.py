...
def get_instances(trainMatrix, user_review_fea, item_review_fea):
    ...
    for (u, i) in trainMatrix.keys():
        if u in user_review_fea.keys() and i in item_review_fea.keys():
            ...
            labels.append(label)
    return np.array(user_input), np.array(user_fea, dtype='float32'), np.array(item_input), np.array(item_fea,
                  dtype='float32'), np.array(labels)
...
# -------------- fit our model ---------------- #
hist = model.fit([user_input, user_fea, item_input, item_fea], labels, batch_size=64, epochs=epochs,
